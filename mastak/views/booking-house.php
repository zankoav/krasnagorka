    ...
    <div class="booking-houses__item">
        ...
        <a href="<?= get_the_permalink(); ?>" target="_blank"
           class="booking-houses__image-wrapper">
            <img class="booking-houses__image"
                 src="<?= get_the_post_thumbnail_url(get_the_ID(), 'calendar-thumb'); ?>"
                 alt="<?= get_the_title(); ?>">
        </a>
        <div class="booking-houses__header">
            <p class="booking-houses__description">
                <?php if (!empty($min_people)): ?>
                    <span class="booking-houses__description-item"
                          data-info="<?= $min_people; ?>"><img
                                class="apartment__icon apartment__icon_mr-4px"
                                src="<?= CORE_PATH ?>assets/icons/min-01.svg"
                                alt="icon">x</span>
                <?php endif;
                    if (!empty($max_people)):?>
                        <span class="booking-houses__description-item" data-info="<?= $max_people; ?>"><img
                                    class="apartment__icon apartment__icon_mr-4px"
                                    src="<?= CORE_PATH ?>assets/icons/max-01.svg"
                                    alt="icon">x</span>
                    <?php endif;
                    if (!empty($double_bed)):?>
                        <span class="booking-houses__description-item" data-info="<?= $double_bed; ?>"><img
                                    class="apartment__icon apartment__icon_mr-4px"
                                    src="<?= CORE_PATH ?>assets/icons/double-king-size-bed.svg"
                                    alt="icon">x</span>
                    <?php endif;
                    if (!empty($single_bed)):?>
                        <span class="booking-houses__description-item" data-info="<?= $single_bed; ?>"><img
                                    class="apartment__icon apartment__icon_mr-4px"
                    ...
                    <?php endif;
                    if (!empty($toilet)):?>
                        <span class="booking-houses__description-item" data-info="<?= $toilet; ?>"><img
                                    class="apartment__icon apartment__icon_mr-4px"
                                    src="<?= CORE_PATH ?>assets/icons/toilet.svg"
                                    alt="icon">x</span>
                    <?php endif;
                    if (!empty($bed_rooms)):?>
                        <span class="booking-houses__description-item" data-info="<?= $bed_rooms; ?>"><img
                                    class="apartment__icon apartment__icon_mr-4px"
                                    src="<?= CORE_PATH ?>assets/icons/br-01.svg"
                                    alt="icon">x</span>
                    <?php endif;
                    if (!empty($triple_bed)):?>
                        <span class="booking-houses__description-item" data-info="<?= $triple_bed; ?>"><img
                                    class="apartment__icon apartment__icon_mr-4px"
        ...
        <div class="booking-houses__calendars">
            <div class="booking-houses__calendars-inner">
                <a href="#" data-calendar='<?= get_post_meta(get_the_ID(), "mastak_house_calendar", true); ?>'
                   class="our-house__button our-house__button--green booking-houses__calendars-button">Показать
                    календарь
                </a>
            </div>
