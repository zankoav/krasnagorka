                    ...
                    <?php
                        ...
                        foreach ((array) $video_rows as $video_row) {
                            if (!empty($video_row['video']) || !empty($video_row['video_file'])) {
                                $video_items[] = $video_row;
                            }
                        }

                        $video_embeds = array();
                        foreach ($video_items as $video_item) {
                            $video_url = !empty($video_item['video']) ? $video_item['video'] : '';
                            $video_file = !empty($video_item['video_file']) ? $video_item['video_file'] : '';
                            $desktop_width = !empty($video_item['desktop_width']) ? (int) $video_item['desktop_width'] : 100;
                            $desktop_width = min(100, max(1, $desktop_width));
                            $embed_html = $video_file
                                ? sprintf('<video controls preload="metadata" style="display:block;width:100%%;height:auto;"><source src="%s" type="video/mp4">Ваш браузер не поддерживает воспроизведение видео.</video>', esc_url($video_file))
                                : mastak_get_video_embed_html($video_url);

                            if ($embed_html) {
                                $video_embeds[] = array(
                                    'html'         => $embed_html,
                                    'is_file'      => (bool) $video_file,
                                ...
                                <div class="accordion-mixed__content-inner">
                                    <style>
                                        ...
                                        @media (min-width: 768px) {
                                            .video_wrapper { max-width: var(--video-desktop-width, 100%); }
                                        }
                                        .video_wrapper--instagram,
                                        .video_wrapper--file { height: auto; padding: 0; }
                                    ...
                                    <?php foreach ($video_embeds as $video_embed) : ?>
                                        <div class="video_wrapper<?= $video_embed['is_instagram'] ? ' video_wrapper--instagram video-tab-wrapper--instagram' : ''; ?><?= $video_embed['is_file'] ? ' video_wrapper--file' : ''; ?>" style="--video-desktop-width: <?= $video_embed['width']; ?>%; margin-left: auto; margin-right: auto;">
