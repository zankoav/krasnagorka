...
<section class="b-container">
    ...
    <div class="base-place b-mb-2">
        <a rel="group" href="<?= wp_get_attachment_image_url($schema_houses_id, 'full'); ?>" class="base-place__image fancybox image">
            <img class="base-place__image-inner" alt="Карта домов" src="<?= wp_get_attachment_image_url($schema_houses_id, $image_size_schema) ?>" srcset="<?= wp_get_attachment_image_srcset($schema_houses_id, $image_size_schema) ?>" sizes="<?= wp_get_attachment_image_sizes($schema_houses_id, $image_size_schema) ?>">
        </a>
    ...
    <div class="base-place">
        <a rel="group" href="<?= wp_get_attachment_image_url($schema_services_id, 'full'); ?>" class="base-place__image fancybox image">
            <img class="base-place__image-inner" alt="Карта услуг" src="<?= wp_get_attachment_image_url($schema_services_id, $image_size_schema) ?>" srcset="<?= wp_get_attachment_image_srcset($schema_services_id, $image_size_schema) ?>" sizes="<?= wp_get_attachment_image_sizes($schema_services_id, $image_size_schema) ?>">
        </a>
        ...
        <div class="footer-bottom-left__socials footer-bottom-left__socials_map">
            <?php if (!empty($instagramm)) : ?>
                <a href="<?= $instagramm; ?>" target="_blank">
                    <img class="footer-bottom-left-img" src="<?= CORE_PATH ?>assets/icons/social/insta_gray.svg" alt="Instagram" title="Instagram">
                </a>
            <?php endif; ?>
            <?php if (!empty($tiktok)) : ?>
                <a href="<?= $tiktok; ?>" target="_blank">
                    <img class="footer-bottom-left-img" src="<?= CORE_PATH ?>assets/icons/social/tiktok_gray.svg" alt="tiktok" title="tiktok">
                </a>
            <?php endif; ?>
            <?php if (!empty($telegram)) : ?>
                <a href="<?= $telegram; ?>" target="_blank">
                    <img class="footer-bottom-left-img" src="<?= CORE_PATH ?>assets/icons/social/telegram_gray.svg" alt="telegram" title="telegram">
                </a>
            <?php endif; ?>
            <?php if (!empty($vk)) : ?>
                <a href="<?= $vk; ?>" target="_blank">
                    <img class="footer-bottom-left-img" src="<?= CORE_PATH ?>assets/icons/social/vk_gray.svg" alt="Вконтакте" title="Вконтакте">
                </a>
            <?php endif; ?>
            <?php if (!empty($youtobe)) : ?>
                <a href="<?= $youtobe; ?>" target="_blank">
                    <img class="footer-bottom-left-img" src="<?= CORE_PATH ?>assets/icons/social/youtube_gray.svg" alt="Youtube" title="Youtube">
                </a>
            <?php endif; ?>
            <?php if (!empty($facebook)) : ?>
                <a href="<?= $facebook; ?>" target="_blank">
                    <img class="footer-bottom-left-img" src="<?= CORE_PATH ?>assets/icons/social/fb_gray.svg" alt="Facebook" title="Facebook">
                </a>
            <?php endif; ?>
            <?php if (!empty($odnoklassniki)) : ?>
                <a href="<?= $odnoklassniki; ?>" target="_blank">
                    <img class="footer-bottom-left-img" src="<?= CORE_PATH ?>assets/icons/social/ok_gray.svg" alt="Одноклассники" title="Одноклассники">
                </a>
            <?php endif; ?>
        </div>
    </div>
